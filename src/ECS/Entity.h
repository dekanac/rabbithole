...
class Entity
{
  ...
  public:
    ...
    template <typename TComponent, typename... TArgs> TComponent& AddComponent(TArgs&&... _mArgs)
    {
        if (HasComponent<TComponent>())
        {
            ASSERT(false,
                   "Attempting to add a component twice! Entity ID: {}, "
                   "ComponentType: {}",
                   m_Id, Component::GetComponentTypeID<TComponent>());
        }
    ...
    template <typename TComponent> void RemoveComponent()
    {
        if (!HasComponent<TComponent>())
        {
            LOG_WARNING("Attempting to remove a component that does not exist! "
                        "Entity ID : {}, ComponentType : {}",
                        m_Id, Component::GetComponentTypeID<TComponent>());
        }

        m_Components.erase(std::remove_if(
            begin(m_Components), end(m_Components), [](Component* component)
            { return component->GetType() == Component::GetComponentTypeID<TComponent>(); }));
    }
};
