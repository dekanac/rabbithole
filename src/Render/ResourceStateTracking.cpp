    ...
    for (auto resource : m_ResourcesForTransition)
    {
        ...
        if (!(resourceState == resourceShouldBe))
        {
            if (resource->GetType() == ResourceType::Texture)
            {
                ...
                renderer.ResourceBarrier(textureResource, resourceState, resourceShouldBe,
                                         resourcePreviousStage, resourceCurrentStage,
            ...
            {
                renderer.ResourceBarrier(static_cast<VulkanBuffer*>(resource), resourceState,
                                         resourceShouldBe, resourcePreviousStage,
