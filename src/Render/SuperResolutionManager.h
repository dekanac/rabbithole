...
class SuperResolutionManager
{
  ...
  public:
    ...
    const std::pair<uint32_t, uint32_t> GetNativeResolution()
    {
        return std::make_pair(m_NativeResolutionWidth, m_NativeResolutionHeight);
    }
    const std::pair<uint32_t, uint32_t> GetUpscaledResolution()
    {
        return std::make_pair(m_UpscaledResolutionWidth, m_UpscaledResolutionHeight);
    }
    ...
    void Draw(VulkanCommandBuffer& commandBuffer, const FfxUpscaleSetup& cameraSetup,
              UIState* pState);
    void GenerateReactiveMask(VulkanCommandBuffer& commandBuffer,
