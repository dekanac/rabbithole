...
VulkanTexture* ResourceManager::CreateSingleMipFromTexture(VulkanDevice& device,
...
VulkanTexture* ResourceManager::CreateTexture(VulkanDevice& device, std::string path,
                                              ROTextureCreateInfo createInfo)
{
    ...
    if (isCubeMap)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            memcpy(texData->pData + i * texData->size / 6, cubeMapData->pData[i]->pData,
                   texData->size / 6);
        }
...
VulkanTexture* ResourceManager::CreateTexture(VulkanDevice& device, const TextureData* data,
                                              ROTextureCreateInfo createInfo)
{
...
bool ResourceManager::CreateShader(VulkanDevice& device, ShaderInfo& createInfo, const char* code,
                                   size_t codeSize, const char* name)
{
