...
{
    if ((resourceState == ResourceState::TransferSrc) ||
        (resourceState == ResourceState::TransferDst))
    {
...
VkBorderColor GetVkBorderColorFrom(const Color color)
{
    ...
    {
        return color.value[3] == 1.0f ? VK_BORDER_COLOR_FLOAT_OPAQUE_BLACK
                                      : VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK;
    }
    else if (color.value[0] == 1.0f && color.value[1] == 1.0f && color.value[2] == 1.0f &&
             color.value[3] == 1.0f)
    {
...
VkPipelineStageFlags GetVkPipelineStageFromResourceStageAndState(const ResourceStage stage,
...
uint64_t GetTextureSizeFrom(Format format, Extent3D textureExtent, uint32_t mipCount,
                            uint32_t arrayCount)
{
    uint64_t size = 0;
    ...
    {
        VkExtent3D extent = {std::max(textureExtent.Width >> i, 1u),
                             std::max(textureExtent.Height >> i, 1u),
                             std::max(textureExtent.Depth >> i, 1u)};
