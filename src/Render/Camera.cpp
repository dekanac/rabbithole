...
Camera::Camera()
    : m_Position(0.0f, 0.0f, 0.0f), m_FieldOfView(45.0f), m_NearPlane(0.1f), m_FarPlane(1000.0f),
      m_Aspect(static_cast<float>(Window::instance().GetExtent().width) /
               Window::instance().GetExtent().height),
      m_Pitch(0.f), m_Yaw(0.f), m_ProjMatrix(Matrix44f{1.f}), m_ViewMatrix(Matrix44f{1.f}),
      m_FocalPoint({0.0f, 0.0f, 0.0f}), m_CameraPanSpeed(0.8f), m_Distance(0.f)
{
    UpdateView();
}
...
void Camera::Update(float dt)
{
    ...
    if (moveUpInput)
    {
        if (cameraFrontBackMovePerc < upperBound)
            cameraFrontBackMovePerc += accelerationStep;
    }
    else
    {
        if (cameraFrontBackMovePerc >= 0)
            cameraFrontBackMovePerc -= breakStep;
    }
    ...
    if (moveLeftInput)
    {
        if (cameraLeftRightMovePerc < upperBound)
            cameraLeftRightMovePerc += accelerationStep;
    }
    else
    {
        if (cameraLeftRightMovePerc >= 0)
            cameraLeftRightMovePerc -= breakStep;
    }
    ...
    m_FocalPoint -= glm::normalize(glm::cross(front, up)) * cameraSpeed *
                    (static_cast<float>(cameraLeftRightMovePerc) / upperBound);
    m_FocalPoint +=
        cameraSpeed * front * (static_cast<float>(cameraFrontBackMovePerc) / upperBound);

    UpdateView();
}

Matrix44f Camera::GetMatrix() const { return m_ProjMatrix * m_ViewMatrix; }

Matrix44f Camera::Projection() const { return m_ProjMatrix; }

Matrix44f Camera::ProjectionJittered() const { return m_ProjMatrixJittered; }

Matrix44f Camera::View() const { return m_ViewMatrix; }

const Vector3f Camera::GetPosition() const { return CalculatePosition(); }

float Camera::GetFieldOfView() const { return m_FieldOfView; }

float Camera::GetFieldOfViewVerticalRad() const { return (glm::radians(m_FieldOfView) / m_Aspect); }

void Camera::SetFieldOfView(float fieldOfView)
{
    ASSERT(fieldOfView > 0.0f && fieldOfView < 180.0f,
           "Field of view mus be between 0 adn 180 degrees!");
...
Quaternion Camera::GetOrientation() const { return Quaternion(Vector3f(-m_Pitch, -m_Yaw, 0.0f)); }
