...
bool Renderer::Init()
{
    ...
    OPTICK_GPU_INIT_VULKAN(&graphicsDevice, &physicalDevice, &graphicsQueue, &graphicsFamily, 1,
                           nullptr);
...
#if defined(VULKAN_HWRT)
    m_VulkanDevice.pfnDestroyAccelerationStructureKHR(m_VulkanDevice.GetGraphicDevice(),
                                                      TLAS.accelerationStructure, nullptr);
    m_VulkanDevice.pfnDestroyAccelerationStructureKHR(m_VulkanDevice.GetGraphicDevice(),
...
void Renderer::DrawFrame()
{
    ...
    result = m_VulkanSwapchain->SubmitCommandBufferAndPresent(GetCurrentCommandBuffer(),
                                                              &m_CurrentImageIndex);
...
{
    g_DefaultWhiteTexture = m_ResourceManager.CreateTexture(
        m_VulkanDevice, m_ResFolder + "/textures/default_white.png",
        ROTextureCreateInfo{
            .flags = {TextureFlags::Color | TextureFlags::Read | TextureFlags::TransferDst},
            .format = {Format::R8G8B8A8_UNORM_SRGB},
            .name = {"Default White"}});

    g_DefaultBlackTexture = m_ResourceManager.CreateTexture(
        m_VulkanDevice, m_ResFolder + "/textures/default_black.png",
        ROTextureCreateInfo{
            .flags = {TextureFlags::Color | TextureFlags::Read | TextureFlags::TransferDst},
            .format = {Format::R8G8B8A8_UNORM_SRGB},
            .name = {"Default Black"}});

    g_Default3DTexture = m_ResourceManager.CreateTexture(
        m_VulkanDevice, RWTextureCreateInfo{.dimensions = {4, 4, 4},
                                            .flags = {TextureFlags::Color | TextureFlags::Read},
                                            .format = {Format::B8G8R8A8_UNORM},
                                            .name = {"Default 3D"}});

    g_DefaultArrayTexture = m_ResourceManager.CreateTexture(
        m_VulkanDevice, RWTextureCreateInfo{.dimensions = {4, 4, 1},
                                            .flags = {TextureFlags::Color | TextureFlags::Read},
                                            .format = {Format::B8G8R8A8_UNORM},
                                            .name = {"Default Array"},
                                            .arraySize = {4}});

    noise2DTexture = m_ResourceManager.CreateTexture(
        m_VulkanDevice, m_ResFolder + "/textures/noise3.png",
        ROTextureCreateInfo{
            .flags = {TextureFlags::Color | TextureFlags::Read | TextureFlags::TransferDst},
            .format = {Format::B8G8R8A8_UNORM},
            .name = {"Noise2D"}});

    blueNoise2DTexture = m_ResourceManager.CreateTexture(
        m_VulkanDevice, m_ResFolder + "/textures/noise.png",
        ROTextureCreateInfo{.flags = {TextureFlags::Color | TextureFlags::Read |
                                      TextureFlags::TransferDst | TextureFlags::Storage},
                            .format = {Format::B8G8R8A8_UNORM},
                            .name = {"BlueNoise2D"}});

    noise3DLUT = m_ResourceManager.CreateTexture(
        m_VulkanDevice, RWTextureCreateInfo{.dimensions = {64, 64, 64},
                                            .flags = {TextureFlags::Color | TextureFlags::Read |
                                                      TextureFlags::Storage},
                                            .format = {Format::R8G8B8A8_UNORM},
                                            .name = {"Noise 3D LUT"}});
}

void Renderer::CalculateMatrices(VulkanglTFModel::Node* node, Vertex* vertexBufferCpu,
                                 uint32_t* indexBufferCpu,
                                 std::vector<bool>& verticesMultipliedWithMatrix)
{
    ...
    {
        for (uint32_t i = primitive.firstIndex; i < primitive.firstIndex + primitive.indexCount;
             i++)
        {
            auto currentIndex = indexBufferCpu[i];
            if (!verticesMultipliedWithMatrix[currentIndex])
            {
                vertexBufferCpu[currentIndex].position =
                    nodeMatrix * Vector4f{vertexBufferCpu[currentIndex].position, 1};
...
void Renderer::LoadModels()
{
    ...
    // RenderingContext::Clouds_Transparent); cloudMeshes.emplace_back(this,
    // m_ResFolder + "/meshes/simpleShapes/box.obj",
    // RenderingContext::Clouds_Transparent);
}
...
void Renderer::EndLabel() { m_VulkanDevice.EndLabel(GetCurrentCommandBuffer()); }
...
void Renderer::DrawGeometryGLTF(std::vector<VulkanglTFModel>& bucket)
{
    ...
    RenderPass* renderpass = m_StateManager.GetRenderPassDirty()
                                 ? m_PipelineManager.FindOrCreateRenderPass(
                                       m_VulkanDevice, attachments, depthStencil, *renderPassInfo)
    ...
    for (auto& model : bucket)
    {
        ...
        model.Draw(GetCurrentCommandBuffer(), m_StateManager.GetPipeline()->GetPipelineLayout(),
                   m_CurrentImageIndex, m_GeometryIndirectDrawBuffer);
    }
...
void Renderer::DrawClouds(std::vector<VulkanglTFModel>& bucket)
{
    ...
    for (auto& model : bucket)
    {
        ...
        model.Draw(GetCurrentCommandBuffer(), m_StateManager.GetPipeline()->GetPipelineLayout(),
                   m_CurrentImageIndex, m_IndirectCloudDrawBuffer);
    }
...
void Renderer::TraceRays(uint32_t x, uint32_t y, uint32_t z)
{
    ...
    m_VulkanDevice.pfnCmdTraceRaysKHR(
        GET_VK_HANDLE(GetCurrentCommandBuffer()), &bindingTables.raygen.stridedDeviceAddressRegion,
        &bindingTables.miss.stridedDeviceAddressRegion,
        &bindingTables.hit.stridedDeviceAddressRegion, &emptySbtEntry, x, y, z);
#endif
}
...
void Renderer::BindPushConstInternal()
{
    if (m_StateManager.ShouldBindPushConst())
    {
        vkCmdPushConstants(GET_VK_HANDLE(GetCurrentCommandBuffer()),
                           ...
                           GetVkShaderStageFrom(m_StateManager.GetPipeline()->GetType()), 0,
...
void Renderer::CopyToSwapChain()
{
    ...
    // if editor is activated render into ImGui image, else render full screen
    // into swapchain
    if (!isInEditorMode)
        vkCmdDraw(GET_VK_HANDLE(GetCurrentCommandBuffer()), 3, 1, 0, 0);
    ...
    if (IsImguiReady())
    {
        if (isInEditorMode)
        {
            ...
            ImGui::Image(
                (ImTextureID)m_ImGuiManager.GetImGuiTextureFrom(TonemappingPass::Output),
                GetScaledSizeWithAspectRatioKept(ImVec2(static_cast<float>(GetUpscaledWidth),
                                                        static_cast<float>(GetUpscaledHeight))));
            ImGui::End();
        }
...
void Renderer::BindCameraMatrices(Camera* camera)
{
    ...
    m_StateManager.UpdateUBOElement(UBOElement::ProjectionMatrix, 4,
                                    &m_CurrentCameraState.ProjectionMatrix);

    m_CurrentCameraState.ProjMatrixJittered = camera->ProjectionJittered();

    m_StateManager.UpdateUBOElement(UBOElement::ProjectionMatrixJittered, 4,
                                    &m_CurrentCameraState.ProjMatrixJittered);

    m_CurrentCameraState.ViewProjMatrix = projection * view;
    m_StateManager.UpdateUBOElement(UBOElement::ViewProjMatrix, 4,
                                    &m_CurrentCameraState.ViewProjMatrix);

    m_CurrentCameraState.CameraPosition = camera->GetPosition();
    m_StateManager.UpdateUBOElement(UBOElement::CameraPosition, 1,
                                    &m_CurrentCameraState.CameraPosition);

    m_CurrentCameraState.ViewInverseMatrix = glm::affineInverse(view);
    m_StateManager.UpdateUBOElement(UBOElement::ViewInverse, 4,
                                    &m_CurrentCameraState.ViewInverseMatrix);

    m_CurrentCameraState.ProjectionInverseMatrix = glm::inverse(projection);
    m_StateManager.UpdateUBOElement(UBOElement::ProjInverse, 4,
                                    &m_CurrentCameraState.ProjectionInverseMatrix);

    m_CurrentCameraState.ViewProjInverseMatrix =
        m_CurrentCameraState.ViewInverseMatrix * m_CurrentCameraState.ProjectionInverseMatrix;
    m_StateManager.UpdateUBOElement(UBOElement::ViewProjInverse, 4,
                                    &m_CurrentCameraState.ViewProjInverseMatrix);

    m_CurrentCameraState.ViewProjJittered =
        m_CurrentCameraState.ViewMatrix * m_CurrentCameraState.ProjMatrixJittered;
    m_StateManager.UpdateUBOElement(UBOElement::ViewProjectionJittered, 4,
                                    &m_CurrentCameraState.ViewProjJittered);

    float width = projection[0][0];
    float height = projection[1][1];

    m_CurrentCameraState.EyeXAxis =
        m_CurrentCameraState.ViewInverseMatrix * Vector4f(-1.0 / width, 0, 0, 0);
    m_CurrentCameraState.EyeYAxis =
        m_CurrentCameraState.ViewInverseMatrix * Vector4f(0, -1.0 / height, 0, 0);
...
void Renderer::BindUBO()
{
    ...
    {
        m_MainConstBuffer[m_CurrentImageIndex]->FillBuffer(m_StateManager.GetUBO(),
...
void Renderer::CreateAccelerationStructure()
{
    ...
    for (auto& model : gltfModels)
    {
        ...
        // TODO: replace this with one Main Init Render command buffer with a
        // lifetime only in init stages
        ...
        VulkanBuffer stagingBuffer(m_VulkanDevice,
                                   BufferCreateInfo{.flags = BufferUsageFlags::TransferDst,
                                                    .memoryAccess = MemoryAccess::CPU,
                                                    .size = modelVertexBuffer->GetSize(),
                                                    .name = "StagingBuffer"});
        m_VulkanDevice.CopyBuffer(tempCommandBuffer, *modelVertexBuffer, stagingBuffer,
                                  modelVertexBuffer->GetSize());
        ...
        VulkanBuffer stagingBuffer2(m_VulkanDevice,
                                    BufferCreateInfo{.flags = BufferUsageFlags::TransferDst,
                                                     .memoryAccess = MemoryAccess::CPU,
                                                     .size = modelIndexBuffer->GetSize(),
                                                     .name = "StagingBuffer"});

        m_VulkanDevice.CopyBuffer(tempCommandBuffer, *modelIndexBuffer, stagingBuffer2,
                                  modelIndexBuffer->GetSize());
    ...
    // BLAS
    {
        ...
        VulkanBuffer* sceneVertexBuffer = m_ResourceManager.CreateBuffer(
            m_VulkanDevice,
            BufferCreateInfo{
                .flags = {BufferUsageFlags::StorageBuffer | BufferUsageFlags::ShaderDeviceAddress |
                          BufferUsageFlags::AccelerationStructureBuild},
                .memoryAccess = {MemoryAccess::GPU},
                .size = {static_cast<uint32_t>(verticesFinal.size() * sizeof(Vector4f))},
                .name = {"VertexBuffer"}});
        sceneVertexBuffer->FillBuffer(
            verticesFinal.data(), static_cast<uint32_t>(verticesFinal.size()) * sizeof(Vector4f));

        VulkanBuffer* sceneIndexBuffer = gltfModels[0].GetIndexBuffer();

        vertexBufferDeviceAddress.deviceAddress =
            RayTracing::GetBufferDeviceAddress(m_VulkanDevice, sceneVertexBuffer);
        indexBufferDeviceAddress.deviceAddress =
            RayTracing::GetBufferDeviceAddress(m_VulkanDevice, sceneIndexBuffer);

        uint32_t numTriangles =
            static_cast<uint32_t>(sceneIndexBuffer->GetSize() / sizeof(uint32_t)) / 3;
        ...
        m_VulkanDevice.pfnGetAccelerationStructureBuildSizesKHR(
            m_VulkanDevice.GetGraphicDevice(), VK_ACCELERATION_STRUCTURE_BUILD_TYPE_DEVICE_KHR,
            &accelerationStructureBuildGeometryInfo, &numTriangles,
            &accelerationStructureBuildSizesInfo);

        RayTracing::CreateAccelerationStructure(this, BLAS,
                                                VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR,
                                                accelerationStructureBuildSizesInfo);

        RayTracing::ScratchBuffer scratchBuffer{};
        scratchBuffer.buffer = m_ResourceManager.CreateBuffer(
            m_VulkanDevice,
            BufferCreateInfo{
                .flags = {BufferUsageFlags::StorageBuffer | BufferUsageFlags::ShaderDeviceAddress},
                .memoryAccess = {MemoryAccess::GPU},
        ...
        scratchBuffer.deviceAddress = RayTracing::AlignedSize(
            RayTracing::GetBufferDeviceAddress(m_VulkanDevice, scratchBuffer.buffer),
            m_VulkanDevice.GetAccelerationStructureProperties()
        ...
        m_VulkanDevice.pfnCmdBuildAccelerationStructuresKHR(
            GET_VK_HANDLE(commandBuffer), 1, &accelerationBuildGeometryInfo,
    ...
    // TLAS
    {
        ...
        VulkanBuffer* instancesBuffer = m_ResourceManager.CreateBuffer(
            m_VulkanDevice,
            BufferCreateInfo{.flags = {BufferUsageFlags::ShaderDeviceAddress |
                                       BufferUsageFlags::AccelerationStructureBuild},
                             .memoryAccess = {MemoryAccess::CPU},
        ...
        instanceDataDeviceAddress.deviceAddress =
            RayTracing::GetBufferDeviceAddress(m_VulkanDevice, instancesBuffer);
        ...
        RayTracing::CreateAccelerationStructure(this, TLAS,
        ...
        scratchBufferTLAS.buffer = m_ResourceManager.CreateBuffer(
            m_VulkanDevice,
            BufferCreateInfo{
                .flags = {BufferUsageFlags::StorageBuffer | BufferUsageFlags::ShaderDeviceAddress},
                .memoryAccess = {MemoryAccess::GPU},
        ...
        scratchBufferTLAS.deviceAddress = RayTracing::AlignedSize(
            RayTracing::GetBufferDeviceAddress(m_VulkanDevice, scratchBufferTLAS.buffer),
            m_VulkanDevice.GetAccelerationStructureProperties()
                .minAccelerationStructureScratchOffsetAlignment);
...
void Renderer::BindDescriptorSets()
{
    VulkanDescriptorSet* descriptorSet =
        m_StateManager.FinalizeDescriptorSet(m_VulkanDevice, m_DescriptorPool.get());
    VulkanPipeline* pipeline = m_StateManager.GetPipeline();

    vkCmdBindDescriptorSets(GET_VK_HANDLE(GetCurrentCommandBuffer()),
                            GetVkBindPointFrom(pipeline->GetType()),
                            GET_VK_HANDLE_PTR(pipeline->GetPipelineLayout()), 0, 1,
                            GET_VK_HANDLE_PTR(descriptorSet), 0, nullptr);
}

void Renderer::CompileShader(const std::string& name, const std::string& entryPoint,
                             std::vector<const char*> defines)
{
    ...
    m_ResourceManager.CreateShader(m_VulkanDevice, createInfo, reinterpret_cast<const char*>(data),
                                   size, fileNameFinal.c_str());
...
void Renderer::CreateCommandBuffers()
{
    ...
    {
        m_MainRenderCommandBuffers[i] =
            std::make_unique<VulkanCommandBuffer>(m_VulkanDevice, "Main Render Command Buffer");
    }
}
...
void Renderer::CreateSwapchain()
{
    ...
    m_VulkanSwapchain = std::make_unique<VulkanSwapchain>(
        m_VulkanDevice,
        Extent2D{static_cast<uint32_t>(currentWidth), static_cast<uint32_t>(currentHeight)});
}
...
void Renderer::RecreateSwapchain()
{
    ...
    m_VulkanSwapchain = std::make_unique<VulkanSwapchain>(
        m_VulkanDevice,
        Extent2D{static_cast<uint32_t>(currentWidth), static_cast<uint32_t>(currentHeight)},
        GET_VK_HANDLE_PTR(m_VulkanSwapchain.get()));
}

void Renderer::ResourceBarrier(VulkanTexture* texture, ResourceState oldLayout,
                               ResourceState newLayout, ResourceStage srcStage,
                               ResourceStage dstStage, uint32_t mipLevel, uint32_t mipCount)
{
    m_VulkanDevice.ResourceBarrier(GetCurrentCommandBuffer(), texture, oldLayout, newLayout,
                                   srcStage, dstStage, mipLevel, mipCount);
}

void Renderer::ResourceBarrier(VulkanBuffer* buffer, ResourceState oldLayout,
                               ResourceState newLayout, ResourceStage srcStage,
                               ResourceStage dstStage)
{
    m_VulkanDevice.ResourceBarrier(GetCurrentCommandBuffer(), buffer, oldLayout, newLayout,
                                   srcStage, dstStage);
}
...
void Renderer::RecordCommandBuffer()
{
    ...
    if (m_ImGuiManager.IsInitialized())
    {
        ...
        m_ImGuiManager.NewFrame(0.f, 0.f, static_cast<float>(Window::instance().GetExtent().width),
                                static_cast<float>(Window::instance().GetExtent().height));
...
void Renderer::CreateUniformBuffers()
{
    for (uint32_t i = 0; i < m_VulkanSwapchain->GetImageCount(); i++)
    {
        m_MainConstBuffer[i] = m_ResourceManager.CreateBuffer(
            m_VulkanDevice, BufferCreateInfo{.flags = {BufferUsageFlags::UniformBuffer},
                                             .memoryAccess = {MemoryAccess::CPU2GPU},
                                             .size = {sizeof(UniformBufferObject)},
                                             .name = {"MainConstantBuffer"}});
    }

    m_VertexUploadBuffer = m_ResourceManager.CreateBuffer(
        m_VulkanDevice, BufferCreateInfo{.flags = {BufferUsageFlags::VertexBuffer},
                                         .memoryAccess = {MemoryAccess::CPU2GPU},
                                         .size = {MB_16},
                                         .name = {"Vertex Upload"}});
}
...
void Renderer::CreateDescriptorPool()
{
    ...
    vulkanDescriptorPoolInfo.DescriptorSizes = {
        uboPoolSize, asPoolSize, cisPoolSize, siPoolSize, sbPoolSize, samImgPoolSize, sPoolSize};

    vulkanDescriptorPoolInfo.MaxSets = 2000;

    m_DescriptorPool =
        std::make_unique<VulkanDescriptorPool>(&m_VulkanDevice, vulkanDescriptorPoolInfo);
}

void Renderer::InitLights()
{
    lights.push_back(LightParams{.position = {70.0f, 200.0f, -100.0f},
                                 .radius = {1.f},
                                 .color = {1.f, 0.6f, 0.2f},
                                 .intensity = {3.f},
    ...
    lights.push_back(LightParams{.position = {-10.f, 1.2f, -3.0f},
                                 .radius = {12.f},
                                 .color = {1.f, 0.1f, 0.1f},
                                 .intensity = {1.f},
    ...
    lights.push_back(LightParams{.position = {-9.6f, 0.4f, 2.3f},
                                 .radius = {12.f},
...
void Renderer::ConstructBVH()
{
    ...
    vertexBuffer = m_ResourceManager.CreateBuffer(
        m_VulkanDevice,
        BufferCreateInfo{.flags = {BufferUsageFlags::StorageBuffer},
                         .memoryAccess = {MemoryAccess::GPU},
                         .size = {static_cast<uint32_t>(verticesFinal.size() * sizeof(Vector4f))},
                         .name = {"VertexBuffer"}});

    trianglesBuffer = m_ResourceManager.CreateBuffer(
        m_VulkanDevice,
        BufferCreateInfo{.flags = {BufferUsageFlags::StorageBuffer},
                         .memoryAccess = {MemoryAccess::GPU},
    ...
    triangleIndxsBuffer = m_ResourceManager.CreateBuffer(
        m_VulkanDevice,
        BufferCreateInfo{.flags = {BufferUsageFlags::StorageBuffer},
                         .memoryAccess = {MemoryAccess::GPU},
    ...
    cfbvhNodesBuffer = m_ResourceManager.CreateBuffer(
        m_VulkanDevice, BufferCreateInfo{.flags = {BufferUsageFlags::StorageBuffer},
                                         .memoryAccess = {MemoryAccess::GPU},
                                         .size = {static_cast<uint32_t>(
                                             nodeNum * sizeof(BVH::CacheFriendlyBVHNode))},
                                         .name = {"CfbvhNodes"}});

    vertexBuffer->FillBuffer(verticesFinal.data(),
                             static_cast<uint32_t>(verticesFinal.size()) * sizeof(Vector4f));
    trianglesBuffer->FillBuffer(triangles.data(),
                                static_cast<uint32_t>(triangles.size()) * sizeof(BVH::Triangle));
...
void Renderer::ImguiProfilerWindow(std::vector<TimeStamp>& timeStamps)
{
    ...
    ImGui::TextWrapped("GPU Adapter: %s",
                       m_VulkanDevice.GetPhysicalDeviceProperties().properties.deviceName);
...
void Renderer::ImGuiTextureDebugger()
{
    ...
    std::vector<std::pair<uint32_t, VulkanTexture*>> textures(texturesMap.begin(),
                                                              texturesMap.end());
    std::sort(textures.begin(), textures.end(),
              [](const std::pair<uint32_t, VulkanTexture*>& a,
                 const std::pair<uint32_t, VulkanTexture*>& b)
              {
    ...
    if (ImGui::BeginCombo(
            "##combo",
            currentTextureSelectedName.c_str())) // The second parameter is the label
                                                 // previewed before opening the combo.
    {
        for (auto& textureIdPair : textures)
        {
            ...
            bool is_selected =
                (currentTextureSelectedID == currentId); // You can store your selection however you
    ...
    if (currentSelectedTexture)
    {
        ...
        ImGui::Image((ImTextureID)m_ImGuiManager.GetImGuiTextureFrom(TextureDebugPass::Output),
                     GetScaledSizeWithAspectRatioKept(ImVec2(textureWidth, textureHeight), 800.f));
    }
...
void Renderer::ClearImage(VulkanTexture* texture, Color clearValue)
{
    ...
    m_VulkanDevice.ResourceBarrier(GetCurrentCommandBuffer(), texture, texture->GetResourceState(),
                                   ResourceState::TransferDst, texture->GetCurrentResourceStage(),
                                   ResourceStage::Transfer);

    VkClearColorValue clearColor{clearValue.value.x, clearValue.value.y, clearValue.value.z,
                                 clearValue.value.w};
    VkImageSubresourceRange range = {VK_IMAGE_ASPECT_COLOR_BIT, 0, texture->GetMipCount(), 0,
                                     texture->GetRegion().Subresource.ArraySize};
    vkCmdClearColorImage(GET_VK_HANDLE(GetCurrentCommandBuffer()),
                         GET_VK_HANDLE_PTR(texture->GetResource()),
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearColor, 1, &range);

    m_VulkanDevice.ResourceBarrier(GetCurrentCommandBuffer(), texture, ResourceState::TransferDst,
                                   currentState, ResourceStage::Transfer,
                                   texture->GetPreviousResourceStage());
}
...
void Renderer::CopyBuffer(VulkanBuffer& src, VulkanBuffer& dst, uint64_t size /*= UINT64_MAX*/,
...
IndexedIndirectBuffer::IndexedIndirectBuffer(VulkanDevice& device, uint32_t numCommands)
    : gpuBuffer(device, BufferCreateInfo{.flags = BufferUsageFlags::IndirectBuffer |
...
IndexedIndirectBuffer::~IndexedIndirectBuffer() { RABBIT_FREE(localBuffer); }
...
void IndexedIndirectBuffer::AddIndirectDrawCommand(VulkanCommandBuffer& commandBuffer,
                                                   IndexIndirectDrawData& drawData)
{
    vkCmdDrawIndexedIndirect(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE(gpuBuffer),
                             currentOffset * sizeof(IndexIndirectDrawData), 1,
...
void IndexedIndirectBuffer::Reset() { currentOffset = 0; }

template <> void Renderer::BindPipeline<GraphicsPipeline>()
{
    m_ResourceStateTrackingManager.CommitBarriers(*this);
...
template <> void Renderer::BindPipeline<ComputePipeline>()
{
    m_ResourceStateTrackingManager.CommitBarriers(*this);
...
template <> void Renderer::BindPipeline<RayTracingPipeline>()
{
    m_ResourceStateTrackingManager.CommitBarriers(*this);
    PipelineInfo* pipelineInfo = m_StateManager.GetPipelineInfo();

    VulkanPipeline* rayTracingPipeline =
        m_StateManager.GetPipelineDirty()
