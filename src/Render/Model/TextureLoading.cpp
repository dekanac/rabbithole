...
TextureData* LoadTextureFromDDSFile(const std::string& path)
{
    ...
    textureData->size =
        static_cast<uint32_t>(GetTexelSizeFrom(GetFormatFrom(dds->ddsHeaderDx10->dxgiFormat)) *
                              dds->dwHeight * dds->dwWidth);
...
TextureData* LoadTextureFromFile(const std::string& path, bool flipY)
{
    ...
    unsigned char* data =
        stbi_load(apsolutePath.c_str(), &width, &height, &numChannels, forceNumChannels);

    ASSERT(data, "Texture loading failed!");

    return new TextureData{.pData = data,
...
{
    // The order is important because in that order are textures uploaded on
    // cubemap
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        ASSERT(i == 0 || (last_height == output->pData[i]->height &&
                          last_width == output->pData[i]->width),
               "[LoadCubemap] Invalid cubemap textures format!");
...
TextureData* LoadEmbeddedTexture(const aiScene* scene, uint32_t index)
{
    const unsigned char* data =
        reinterpret_cast<const unsigned char*>(scene->mTextures[index]->pcData);
    int width, height, channels;
    unsigned char* image = stbi_load_from_memory(data, scene->mTextures[index]->mWidth, &width,
                                                 &height, &channels, STBI_rgb_alpha);

    return new TextureData{.pData = image,
                           .width = static_cast<uint32_t>(width),
                           .height = static_cast<uint32_t>(height),
