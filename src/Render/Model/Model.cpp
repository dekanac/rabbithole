...
{
    Matrix44f glmMatrix(aiMatrix.a1, aiMatrix.b1, aiMatrix.c1, aiMatrix.d1, aiMatrix.a2,
                        aiMatrix.b2, aiMatrix.c2, aiMatrix.d2, aiMatrix.a3, aiMatrix.b3,
                        aiMatrix.c3, aiMatrix.d3, aiMatrix.a4, aiMatrix.b4, aiMatrix.c4,
                        aiMatrix.d4);
...
VulkanglTFModel::VulkanglTFModel(Renderer* renderer, std::string filename, RenderingContext context,
                                 bool flipNormalY)
    : m_Renderer(renderer), m_Path(filename), m_FlipNormalY(flipNormalY),
...
void VulkanglTFModel::LoadModelFromFile(std::string& filename)
{
    ...
    m_VertexBuffer = resourceManager.CreateBuffer(
        device,
        BufferCreateInfo{.flags = {BufferUsageFlags::VertexBuffer | BufferUsageFlags::TransferSrc |
    ...
    m_IndexBuffer = resourceManager.CreateBuffer(
        device,
        BufferCreateInfo{.flags = {BufferUsageFlags::IndexBuffer | BufferUsageFlags::TransferSrc |
                                   BufferUsageFlags::ShaderDeviceAddress |
                                   BufferUsageFlags::AccelerationStructureBuild |
                                   BufferUsageFlags::StorageBuffer},
                         .memoryAccess = {MemoryAccess::GPU},
...
void VulkanglTFModel::LoadNode(const aiNode* inputNode, const aiScene* inputScene,
    ...
    if (inputNode->mNumMeshes > 0)
    {
        for (uint32_t m = 0; m < inputNode->mNumMeshes; m++)
        {
            ...
            for (size_t v = 0; v < vertexCount; v++)
            {
                ...
                vert.normal = glm::normalize(Vector3f(
                    normalsBuffer ? glm::make_vec3(&normalsBuffer[v * 3]) : Vector3f(0.0f)));
                if (m_FlipNormalY)
                {
                    vert.normal.y = -vert.normal.y;
                }
                vert.uv =
                    texCoordsBuffer ? glm::make_vec2(&texCoordsBuffer[v * 3]) : Vector2f(0.0f);
                vert.tangent =
                    tangentBuffer ? glm::make_vec3(&tangentBuffer[v * 3]) : Vector3f(0.0f);
...
void VulkanglTFModel::LoadMaterials(const aiScene* input)
{
    ...
    for (uint32_t i = 0; i < numOfMaterials; i++)
    {
        ...
        m_Materials[i].baseColorFactor =
            Vector4f{baseColor.r, baseColor.g, baseColor.b, baseColor.a};

        aiColor3D emissiveIntensity;
        currentMaterial->Get(AI_MATKEY_COLOR_EMISSIVE, emissiveIntensity);
        m_Materials[i].emissiveColorAndStrenght =
            Vector4f{emissiveIntensity.r, emissiveIntensity.b, emissiveIntensity.g, 1.f};
        ...
        if (textureCount > 0)
        {
            ...
            if (texturePath.length > 0 && texturePath.data[0] == '*')
            {
                TextureLoading::TextureData* textureData =
                    TextureLoading::LoadEmbeddedTexture(input, textureAndIndex.second);

                baseColorTexture = Renderer::instance().GetResourceManager().CreateTexture(
                    device, textureData,
                    ROTextureCreateInfo{
                        .flags = {TextureFlags::Color | TextureFlags::Read |
                                  TextureFlags::TransferDst | TextureFlags::TransferSrc},
        ...
        if (textureCount > 0)
        {
            ...
            if (texturePath.length > 0 && texturePath.data[0] == '*')
            {
                ...
                metallicRoughnessTexture = Renderer::instance().GetResourceManager().CreateTexture(
                    device, textureData,
                    ROTextureCreateInfo{
                        .flags = {TextureFlags::Color | TextureFlags::Read |
                                  TextureFlags::TransferDst | TextureFlags::TransferSrc},
            ...
            m_Materials[i].metallicRoughnessTextureIndex =
                static_cast<int32_t>(m_Textures.size()) - 1;
        }
        ...
        if (textureCount > 0)
        {
            ...
            if (texturePath.length > 0 && texturePath.data[0] == '*')
            {
                ...
                normalTexture = Renderer::instance().GetResourceManager().CreateTexture(
                    device, textureData,
                    ROTextureCreateInfo{
                        .flags = {TextureFlags::Color | TextureFlags::Read |
                                  TextureFlags::TransferDst | TextureFlags::TransferSrc},
...
void VulkanglTFModel::DrawNode(VulkanCommandBuffer& commandBuffer,
                               const VulkanPipelineLayout* pipelineLayout,
                               VulkanglTFModel::Node node, uint8_t backBufferIndex,
                               IndexedIndirectBuffer* indirectBuffer)
{
    if (node.mesh.primitives.size() > 0)
    {
        ...
        for (VulkanglTFModel::Primitive& primitive : node.mesh.primitives)
        {
            ...
            pushData.useAlbedoMap =
                (uint32_t)(m_Materials[primitive.materialIndex].baseColorTextureIndex != -1);
            pushData.useNormalMap =
                (uint32_t)(m_Materials[primitive.materialIndex].normalTextureIndex != -1);
            pushData.useMetallicRoughnessMap =
                (uint32_t)(m_Materials[primitive.materialIndex].metallicRoughnessTextureIndex !=
                           -1);
            pushData.baseColor = m_Materials[primitive.materialIndex].baseColorFactor;
            pushData.emmisiveColorAndStrength =
                m_Materials[primitive.materialIndex].emissiveColorAndStrenght;

            vkCmdPushConstants(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE_PTR(pipelineLayout),
                               VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                               sizeof(ModelDrawData), &pushData);

            // TODO: decrease num of descriptor set binding to number of
            // different materials sort primitives by materialIndexNumber
            if (primitive.indexCount > 0)
            {
                VulkanDescriptorSet* materialDescriptorSet =
                    m_Materials[primitive.materialIndex].materialDescriptorSet[backBufferIndex];
                // Bind the descriptor for the current primitive's texture
                vkCmdBindDescriptorSets(GET_VK_HANDLE(commandBuffer),
                                        VK_PIPELINE_BIND_POINT_GRAPHICS,
                                        GET_VK_HANDLE_PTR(pipelineLayout), 0, 1,
                                        GET_VK_HANDLE_PTR(materialDescriptorSet), 0, nullptr);
...
void VulkanglTFModel::Draw(VulkanCommandBuffer& commandBuffer,
                           const VulkanPipelineLayout* pipeLayout, uint8_t backBufferIndex,
                           IndexedIndirectBuffer* indirectBuffer)
{
...
void VulkanglTFModel::BindBuffers(VulkanCommandBuffer& commandBuffer)
{
    ...
    vkCmdBindIndexBuffer(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE_PTR(m_IndexBuffer), 0,
...
void VulkanglTFModel::CreateDescriptorSet(uint32_t imageIndex)
{
    ...
    if (m_RenderingContext == RenderingContext::GBuffer_Opaque)
    {
        ...
        VulkanDescriptorSetLayout descrSetLayout(
            &device, {m_Renderer->GetShader("VS_GBuffer"), m_Renderer->GetShader("FS_GBuffer")},
            "GeometryDescSetLayout");
        ...
        for (size_t i = 0; i < m_Materials.size(); i++)
        {
            ...
            VulkanTexture* albedo =
                modelMaterial.baseColorTextureIndex != -1 && m_Textures.size() > 0
            ...
            VulkanTexture* metallicRoughness =
                modelMaterial.metallicRoughnessTextureIndex != -1 && m_Textures.size() > 0
            ...
            VulkanDescriptorSet* descriptorSet = pipelineManager.FindOrCreateDescriptorSet(
                device, &pool, &descrSetLayout,
                {uboDescriptor, albedoDescriptor, normalDescriptor, metallicRoughnessDescriptor});
    ...
    else if (m_RenderingContext == RenderingContext::Clouds_Transparent)
    {
        ...
        for (size_t i = 0; i < m_Materials.size(); i++)
        {
            ...
            VulkanDescriptorSet* descriptorSet = pipelineManager.FindOrCreateDescriptorSet(
                device, &pool, &descrSetLayout, {uboDescriptor});
