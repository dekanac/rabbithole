...
void CopyToSwapchainPass::Setup()
{
    ...
    m_Renderer.BindViewport(0, 0, static_cast<float>(renderWindowWidth),
                            static_cast<float>(renderWindowHeight));
...
void CopyToSwapchainPass::Render() { m_Renderer.CopyToSwapChain(); }
...
void OutlineEntityPass::DeclareResources()
{
    ...
    HelperBuffer = m_Renderer.GetResourceManager().CreateBuffer(
        m_Renderer.GetVulkanDevice(),
        BufferCreateInfo{.flags = {BufferUsageFlags::StorageBuffer},
                         .memoryAccess = {MemoryAccess::CPU2GPU}, // TODO: this should
                                                                  // probably be GPU2CPU
...
void OutlineEntityPass::Render() { m_Renderer.DrawFullScreenQuad(); }

void TextureDebugPass::DeclareResources()
{
    Output = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {GetNativeWidth, GetNativeHeight, 1},
    ...
    ParamsGPU = m_Renderer.GetResourceManager().CreateBuffer(
...
void TextureDebugPass::Setup()
{
    ...
    VulkanTexture* selectedTexture =
        m_Renderer.GetTextureWithID(m_Renderer.currentTextureSelectedID);
...
void TextureDebugPass::Render()
{
    ...
    m_Renderer.ResourceBarrier(TextureDebugPass::Output, ResourceState::RenderTarget,
                               ResourceState::GenericRead, ResourceStage::Graphics,
                               ResourceStage::Graphics);
}
