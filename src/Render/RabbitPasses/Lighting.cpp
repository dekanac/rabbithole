...
{
    MainLighting = m_Renderer.GetResourceManager().CreateTexture(
        ...
        RWTextureCreateInfo{.dimensions = {GetNativeWidth, GetNativeHeight, 1},
                            .flags = {TextureFlags::RenderTarget | TextureFlags::Read |
                                      TextureFlags::TransferSrc | TextureFlags::Storage},
    ...
    LightParamsGPU = m_Renderer.GetResourceManager().CreateBuffer(
        m_Renderer.GetVulkanDevice(),
...
void LightingPass::Setup()
{
    ...
    LightingPass::LightParamsGPU->FillBuffer(m_Renderer.lights.data(),
                                             sizeof(LightParams) * numOfLights);
...
void LightingPass::Render() { m_Renderer.DrawFullScreenQuad(); }