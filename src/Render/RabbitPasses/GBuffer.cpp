...
{
    Albedo = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {GetNativeWidth, GetNativeHeight, 1},
                            .flags = {TextureFlags::RenderTarget | TextureFlags::Read},
                            .format = {Format::R8G8B8A8_UNORM},
                            .name = {"GBuffer Albedo"}});

    Emissive = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {GetNativeWidth, GetNativeHeight, 1},
                            .flags = {TextureFlags::RenderTarget | TextureFlags::Read},
                            .format = {Format::R8G8B8A8_UNORM},
    ...
    Normals = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{
    ...
    WorldPosition = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{
            .dimensions = {GetNativeWidth, GetNativeHeight, 1},
            .flags = {TextureFlags::RenderTarget | TextureFlags::Read | TextureFlags::Storage},
    ...
    Velocity = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{
            .dimensions = {GetNativeWidth, GetNativeHeight, 1},
            .flags = {TextureFlags::RenderTarget | TextureFlags::Read | TextureFlags::Storage},
    ...
    Depth = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{
            .dimensions = {GetNativeWidth, GetNativeHeight, 1},
...
void GBufferPass::Setup()
{
    ...
    m_Renderer.BindViewport(0, 0, static_cast<float>(GetNativeWidth),
                            static_cast<float>(GetNativeHeight));
...
void GBufferPass::Render() { m_Renderer.DrawGeometryGLTF(m_Renderer.gltfModels); }

void CopyDepthPass::DeclareResources()
{
    DepthR32 = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {GetNativeWidth, GetNativeHeight, 1},
                            .flags = {TextureFlags::Read | TextureFlags::Storage |
                                      TextureFlags::RenderTarget | TextureFlags::TransferSrc},
...
void CopyDepthPass::Render() { m_Renderer.DrawFullScreenQuad(); }

float skyboxVertices[] = {
    // positions				//for now i got only one
    // possible vertex binding and that is 3, 3, 3, 2 TODO: clean this
...
{
    Main = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        m_Renderer.GetResFolderPath() + "\\textures\\skybox\\skybox.jpg",
        ROTextureCreateInfo{.flags = {TextureFlags::Color | TextureFlags::Read |
                                      TextureFlags::CubeMap | TextureFlags::TransferDst},
...
{
    // TODO: dont do like this, make one big DEVICE LOCAL buffer and calculate
    // offsets
