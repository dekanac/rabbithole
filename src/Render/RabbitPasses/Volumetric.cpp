...
defineResource(VolumetricPass, ParamsGPU,
               VulkanBuffer) VolumetricPass::VolumetricFogParams VolumetricPass::ParamsCPU = {};
...
{
    MediaDensity = m_Renderer.GetResourceManager().CreateTexture(
...
void Create3DNoiseTexturePass::Render()
{
    ...
    m_Renderer.Dispatch(GetCSDispatchCount(width, 8), GetCSDispatchCount(height, 8),
                        GetCSDispatchCount(depth, 8));
}

void ComputeScatteringPass::DeclareResources()
{
    LightScattering = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{
            .dimensions = {g_VolumetricTexX, g_VolumetricTexY, g_VolumetricTexZ},
            .flags = {TextureFlags::Read | TextureFlags::TransferSrc | TextureFlags::Storage},
...
void ApplyVolumetricFogPass::Render() { m_Renderer.DrawFullScreenQuad(); }
...
void VolumetricCloudsPass::Setup()
{
    ...
    if (m_Renderer.IsImguiReady())
    {
        ...
        ImGui::SliderFloat("VolumeLightAbsorption: ", &(fogParams.VolumeLightAbsorption), 0.1f,
                           1.f);
...
void VolumetricCloudsPass::Render() { m_Renderer.DrawFullScreenQuad(); }
