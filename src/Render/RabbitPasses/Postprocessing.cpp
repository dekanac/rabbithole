...
{
    BloomApplied = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {GetUpscaledWidth, GetUpscaledHeight, 1},
    ...
    Downsampled = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {GetUpscaledWidth, GetUpscaledHeight, 1},
    ...
    {
        m_DownsampledMipChain.push_back(m_Renderer.GetResourceManager().CreateSingleMipFromTexture(
            m_Renderer.GetVulkanDevice(), Downsampled, i));
    }

    BloomParamsGPU = m_Renderer.GetResourceManager().CreateBuffer(
...
void BloomCompute::Setup()
{
    ...
    m_Renderer.BindViewport(0, 0, static_cast<float>(GetUpscaledWidth),
...
void TonemappingPass::Render() { m_Renderer.DrawFullScreenQuad(); }
