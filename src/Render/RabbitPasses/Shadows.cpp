...
defineResourceArray(ShadowDenoiseTileClassificationPass, Moments0, VulkanTexture,
                    MAX_NUM_OF_LIGHTS);
defineResourceArray(ShadowDenoiseTileClassificationPass, Moments1, VulkanTexture,
                    MAX_NUM_OF_LIGHTS);
defineResourceArray(ShadowDenoiseTileClassificationPass, Reprojection0, VulkanTexture,
                    MAX_NUM_OF_LIGHTS);
defineResourceArray(ShadowDenoiseTileClassificationPass, Reprojection1, VulkanTexture,
                    MAX_NUM_OF_LIGHTS);
defineResourceArray(ShadowDenoiseTileClassificationPass, TileMetadata, VulkanBuffer,
                    MAX_NUM_OF_LIGHTS);
...
{
    VolumetricShadows = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {g_VolumetricTexX, g_VolumetricTexY, g_VolumetricTexZ},
...
void RTShadowsPass::DeclareResources()
{
    ...
    ShadowMask = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(), RWTextureCreateInfo{
...
void ShadowDenoisePrePass::DeclareResources()
{
    ...
    BufferDimensions = m_Renderer.GetResourceManager().CreateBuffer(
        m_Renderer.GetVulkanDevice(), BufferCreateInfo{.flags = {BufferUsageFlags::UniformBuffer},
    ...
    {
        ShadowData[i] = m_Renderer.GetResourceManager().CreateBuffer(
            m_Renderer.GetVulkanDevice(),
...
void ShadowDenoiseTileClassificationPass::DeclareResources()
{
    ...
    LastFrameDepth = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
        RWTextureCreateInfo{.dimensions = {RTShadowsPass::ShadowResX, RTShadowsPass::ShadowResY, 1},
                            .flags = {TextureFlags::Read | TextureFlags::TransferDst},
                            .format = {Format::R32_SFLOAT},
                            .name = {"Last Frame DepthR32"}});

    ReprojectionInfo = m_Renderer.GetResourceManager().CreateBuffer(
        m_Renderer.GetVulkanDevice(), BufferCreateInfo{.flags = {BufferUsageFlags::UniformBuffer},
                                                       .memoryAccess = {MemoryAccess::CPU2GPU},
    ...
    for (uint32_t i = 0; i < MAX_NUM_OF_LIGHTS; i++)
    {
        ...
        TileMetadata[i] = m_Renderer.GetResourceManager().CreateBuffer(
            m_Renderer.GetVulkanDevice(),
            BufferCreateInfo{
        ...
        // classify
        Moments0[i] = m_Renderer.GetResourceManager().CreateTexture(
            m_Renderer.GetVulkanDevice(),
            RWTextureCreateInfo{
        ...
        Moments1[i] = m_Renderer.GetResourceManager().CreateTexture(
            m_Renderer.GetVulkanDevice(),
            RWTextureCreateInfo{
                .dimensions = {RTShadowsPass::ShadowResX, RTShadowsPass::ShadowResY, 1},
                .flags = {TextureFlags::Read | TextureFlags::Storage},
        ...
        Reprojection0[i] = m_Renderer.GetResourceManager().CreateTexture(
            m_Renderer.GetVulkanDevice(),
            RWTextureCreateInfo{
                .dimensions = {RTShadowsPass::ShadowResX, RTShadowsPass::ShadowResY, 1},
                .flags = {TextureFlags::Read | TextureFlags::Storage},
        ...
        Reprojection1[i] = m_Renderer.GetResourceManager().CreateTexture(
            m_Renderer.GetVulkanDevice(),
            RWTextureCreateInfo{
                .dimensions = {RTShadowsPass::ShadowResX, RTShadowsPass::ShadowResY, 1},
                .flags = {TextureFlags::Read | TextureFlags::Storage},
...
void ShadowDenoiseTileClassificationPass::Setup()
{
    ...
    shadowData.ReprojectionMatrix =
        cameraState.ProjectionMatrix *
        (cameraState.PrevViewMatrix * cameraState.ViewProjInverseMatrix);
...
void ShadowDenoiseTileClassificationPass::ClassifyTiles(uint32_t shadowSlice)
{
    ...
    SetSampledImage(9, GetCurrentIDFromFrameIndex(0)
                           ? ShadowDenoiseTileClassificationPass::Moments0[shadowSlice]
                           : ShadowDenoiseTileClassificationPass::Moments1[shadowSlice]);
    SetStorageImageReadWrite(10, GetCurrentIDFromFrameIndex(1)
                                     ? ShadowDenoiseTileClassificationPass::Moments0[shadowSlice]
                                     : ShadowDenoiseTileClassificationPass::Moments1[shadowSlice]);
...
{
    FilterData = m_Renderer.GetResourceManager().CreateBuffer(
        m_Renderer.GetVulkanDevice(),
        BufferCreateInfo{.flags = {BufferUsageFlags::UniformBuffer},
                         .memoryAccess = {MemoryAccess::CPU2GPU},
...
void ShadowDenoiseFilterPass::Setup()
{
    ...
    m_Renderer.CopyImage(CopyDepthPass::DepthR32,
                         ShadowDenoiseTileClassificationPass::LastFrameDepth);
