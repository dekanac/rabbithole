...
void SSAOPass::DeclareResources()
{
    ...
    {
        Vector4f sample(randomFloats(generator) * 2.0 - 1.0, randomFloats(generator) * 2.0 - 1.0,
    ...
    Samples = m_Renderer.GetResourceManager().CreateBuffer(
        m_Renderer.GetVulkanDevice(), BufferCreateInfo{.flags = {BufferUsageFlags::UniformBuffer},
                                                       .memoryAccess = {MemoryAccess::CPU2GPU},
                                                       .size = {bufferSize},
                                                       .name = {"SSAO Samples"}});
...
{
    BluredOutput = m_Renderer.GetResourceManager().CreateTexture(
        m_Renderer.GetVulkanDevice(),
...
void SSAOBlurPass::Render() { m_Renderer.DrawFullScreenQuad(); }