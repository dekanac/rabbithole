...
VulkanRenderPass::VulkanRenderPass(const VulkanDevice* device,
                                   ...
                                   const VulkanRenderPassInfo& info, const char* name)
    : m_VulkanDevice(device), m_Info(info)
{
    ...
    for (uint32_t i = 0; i < colorAttachmentCount; ++i)
    {
        ...
        attachmentDescription.samples =
            GetVkSampleFlagsFrom(renderTargetView->GetInfo().Resource->GetInfo().MultisampleType);
    ...
    if (depthStencilView)
    {
        ...
        bool readDepthOrStencil =
            IsFlagSet(depthStencilView->GetInfo().Flags & ImageViewFlags::ReadDepth) ||
            IsFlagSet(depthStencilView->GetInfo().Flags & ImageViewFlags::ReadStencil);
        ResourceState depthStencilState =
            readDepthOrStencil ? ResourceState::DepthStencilRead : ResourceState::DepthStencilWrite;
        ...
        attachmentDescription.samples =
            GetVkSampleFlagsFrom(depthStencilView->GetInfo().Resource->GetInfo().MultisampleType);
    ...
    subpassDescription.pColorAttachments =
        colorAttachmentCount ? colorAttachmentReferences.data() : nullptr;
    subpassDescription.pDepthStencilAttachment =
        depthStencilView ? &depthStencilAttachmentReference : nullptr;
    ...
    VULKAN_API_CALL(vkCreateRenderPass(m_VulkanDevice->GetGraphicDevice(), &renderPassCreateInfo,
