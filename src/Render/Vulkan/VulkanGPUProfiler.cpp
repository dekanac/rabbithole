...
void GPUTimeStamps::OnCreate(VulkanDevice* device, uint32_t numberOfBackBuffers)
{
    ...
    VULKAN_API_CALL(
        vkCreateQueryPool(m_Device->GetGraphicDevice(), &queryPoolCreateInfo, NULL, &m_QueryPool));
}
...
void GPUTimeStamps::GetTimeStamp(VulkanCommandBuffer& cmd_buf, const char* label)
{
    ...
    vkCmdWriteTimestamp(GET_VK_HANDLE(cmd_buf), VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, m_QueryPool,
                        offset);
...
void GPUTimeStamps::OnBeginFrame(VulkanCommandBuffer& cmd_buf, std::vector<TimeStamp>* pTimestamps)
{
    ...
    if (measurements > 0)
    {
        double microsecondsPerTick =
            (1e-3f * m_Device->GetPhysicalDeviceProperties().properties.limits.timestampPeriod);
        {
            uint64_t TimingsInTicks[256] = {};
            VULKAN_API_CALL(vkGetQueryPoolResults(m_Device->GetGraphicDevice(), m_QueryPool, offset,
                                                  measurements, measurements * sizeof(uint64_t),
                                                  &TimingsInTicks, sizeof(uint64_t),
                                                  VK_QUERY_RESULT_64_BIT));

            for (uint32_t i = 1; i < measurements; i++)
            {
                TimeStamp ts = {m_labels[m_Frame][i],
                                float(microsecondsPerTick *
                                      (double)(TimingsInTicks[i] - TimingsInTicks[i - 1]))};
                pTimestamps->push_back(ts);
            }

            TimeStamp ts = {"Total GPU Time",
                            float(microsecondsPerTick *
                                  (double)(TimingsInTicks[measurements - 1] - TimingsInTicks[0]))};
            pTimestamps->push_back(ts);
        }
    }
...
void GPUTimeStamps::OnEndFrame() { m_Frame = (m_Frame + 1) % m_NumberOfBackBuffers; }
