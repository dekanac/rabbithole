...
void VulkanPipeline::DefaultPipelineInfo(PipelineInfo*& pipelineInfo, uint32_t width,
                                         uint32_t height)
{
...
void VulkanPipeline::CreateComputePipeline()
{
    ...
    m_DescriptorSetLayout =
        new VulkanDescriptorSetLayout(&m_VulkanDevice, {m_PipelineInfo.computeShader}, "Main");
    ...
    m_PipelineLayout = new VulkanPipelineLayout(m_VulkanDevice, descSetLayouts,
                                                m_PipelineInfo.computeShader->GetPushConstants());
    ...
    VULKAN_API_CALL(vkCreateComputePipelines(m_VulkanDevice.GetGraphicDevice(), VK_NULL_HANDLE, 1,
                                             &computePipelineInfo, nullptr, &m_Pipeline));
}
...
void VulkanPipeline::CreateRayTracingPipeline()
{
    ...
    m_DescriptorSetLayout = new VulkanDescriptorSetLayout(&m_VulkanDevice, {raytracingShaders},
    ...
    VULKAN_API_CALL(m_VulkanDevice.pfnCreateRayTracingPipelinesKHR(
        m_VulkanDevice.GetGraphicDevice(), VK_NULL_HANDLE, VK_NULL_HANDLE, 1, &rayTracingPipelineCI,
        nullptr, &m_Pipeline));

    auto shaderGroupHandleSize = m_VulkanDevice.GetRayTracingProperties().shaderGroupHandleSize;
    auto shaderGroupHandleAlignment =
        m_VulkanDevice.GetRayTracingProperties().shaderGroupHandleAlignment;
    const uint64_t handleSize = shaderGroupHandleSize;
    const uint64_t handleSizeAligned =
        RayTracing::AlignedSize(shaderGroupHandleSize, shaderGroupHandleAlignment);
    const uint32_t groupCount = static_cast<uint32_t>(shaderGroups.size());
    const uint64_t sbtSize = groupCount * handleSizeAligned;

    std::vector<uint8_t> shaderHandleStorage(sbtSize);
    VULKAN_API_CALL(m_VulkanDevice.pfnGetRayTracingShaderGroupHandlesKHR(
        m_VulkanDevice.GetGraphicDevice(), m_Pipeline, 0, groupCount, sbtSize,
    ...
    memcpy(m_BindingTables.raygen.buffer->GetHostVisibleData(), shaderHandleStorage.data(),
           m_BindingTables.raygen.buffer->GetSize());
    memcpy(m_BindingTables.hit.buffer->GetHostVisibleData(),
...
void VulkanPipeline::CreateGraphicsPipeline()
{
    ...
    m_DescriptorSetLayout = new VulkanDescriptorSetLayout(
        &m_VulkanDevice, {m_PipelineInfo.pixelShader, m_PipelineInfo.vertexShader}, "Main");

    std::vector<VulkanDescriptorSetLayout*> descSetLayouts;
    descSetLayouts.push_back(m_DescriptorSetLayout);

    std::vector<VkPushConstantRange> pushConsts{};
    pushConsts.insert(pushConsts.end(), m_PipelineInfo.vertexShader->GetPushConstants().begin(),
                      m_PipelineInfo.vertexShader->GetPushConstants().end());
    pushConsts.insert(pushConsts.end(), m_PipelineInfo.pixelShader->GetPushConstants().begin(),
                      m_PipelineInfo.pixelShader->GetPushConstants().end());
    ...
    VULKAN_API_CALL(vkCreateGraphicsPipelines(m_VulkanDevice.GetGraphicDevice(), VK_NULL_HANDLE, 1,
...
// &get_st(vertexBinding)->m_VertexInputBindingDescription;
// 	m_VertexInputStateCreateInfo.vertexAttributeDescriptionCount =
// static_cast<uint32_t>(get_st(vertexBinding)->m_VertexAttributeDescriptions.size());
// 	m_VertexInputStateCreateInfo.pVertexAttributeDescriptions =
// get_st(vertexBinding)->m_VertexAttributeDescriptions.data();
// }
...
void PipelineInfo::SetMultisampleType(const MultisampleType multiSampleType)
{
    ...
    multisampleInfo.sampleShadingEnable =
        multiSampleType == MultisampleType::Sample_1 ? VK_FALSE : VK_TRUE;
}
...
void PipelineInfo::SetDepthBias(const float depthBiasSlope, const float depthBias,
...
void PipelineInfo::SetStencilOperation(const StencilOperation stencilFail,
...
void PipelineInfo::SetStencilFunction(const CompareOperation compareOperation,
...
void PipelineInfo::SetCullMode(const CullMode mode)
{
    rasterizationInfo.cullMode = mode == CullMode::None    ? VK_CULL_MODE_NONE
                                 : mode == CullMode::Front ? VK_CULL_MODE_FRONT_BIT
                                                           : VK_CULL_MODE_BACK_BIT;
}

void PipelineInfo::SetWindingOrder(const WindingOrder winding)
{
    rasterizationInfo.frontFace = winding == WindingOrder::Clockwise
...
void PipelineInfo::SetColorWriteMask(const uint32_t mrtIndex, const uint32_t mrtCount,
...
void PipelineInfo::SetAttachmentCount(const uint32_t attachmentCount)
{
    ASSERT(attachmentCount <= MaxRenderTargetCount,
           "mrtIndex must be lower then MaxRenderTargetCount");
...
void PipelineInfo::SetAlphaBlendFunction(const uint32_t mrtIndex, const BlendValue srcBlend,
                                         const BlendValue dstBlend)
{
    ASSERT(mrtIndex < MaxRenderTargetCount, "mrtIndex must be lower then MaxRenderTargetCount");
...
void PipelineInfo::SetAlphaBlendFunction(const uint32_t mrtIndex, const BlendValue srcColorBlend,
                                         const BlendValue dstColorBlend,
                                         const BlendValue srcAlphaBlend,
...
void PipelineInfo::SetAlphaBlendOperation(const uint32_t mrtIndex,
...
VulkanPipelineLayout::VulkanPipelineLayout(
    VulkanDevice& device, std::vector<VulkanDescriptorSetLayout*>& descriptorSetLayouts,
    ...
    : m_Device(device)
{
    ...
    VULKAN_API_CALL(vkCreatePipelineLayout(m_Device.GetGraphicDevice(), &pipelineLayoutCreateInfo,
