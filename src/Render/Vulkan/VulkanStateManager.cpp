...
void VulkanStateManager::SetAccelerationStructure(uint32_t slot,
                                                  RayTracing::AccelerationStructure* as)
{
    VulkanDescriptorInfo info{};
    info.Binding = slot;
...
VulkanDescriptorSet* VulkanStateManager::FinalizeDescriptorSet(VulkanDevice& device,
                                                               const VulkanDescriptorPool* pool)
{
    ...
    VulkanDescriptorSet* descriptorset = pipelineManager.FindOrCreateDescriptorSet(
        device, pool, m_Pipeline->GetDescriptorSetLayout(), m_Descriptors);
    m_Descriptors.clear();
