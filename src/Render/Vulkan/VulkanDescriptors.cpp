...
VulkanDescriptor::VulkanDescriptor(const VulkanDescriptorInfo& info) : m_Info(info)
{
    switch (m_Info.Type)
    {
    case DescriptorType::CombinedSampler:
    {
        m_ResourceInfo.m_ResourceInfo.ImageInfo.imageLayout =
    ...
    {
        m_ResourceInfo.m_ResourceInfo.AccelerationStructureHandle =
            m_Info.accelerationStructure->accelerationStructure;
        break;
    }
#endif
    default:
...
VulkanDescriptorPool::VulkanDescriptorPool(const VulkanDevice* device,
                                           const VulkanDescriptorPoolInfo& info)
    : m_Device(device), m_Info(info)
{
    ...
    VULKAN_API_CALL(vkCreateDescriptorPool(device->GetGraphicDevice(), &descriptorPoolCreateInfo,
    ...
    : m_Device(device)
{
    ...
    // doing this to compact bindings (if we have 2 similar bindings from
    // different shaders just merge their stage flags on the same binding slot)
    ...
    {
        for (const VkDescriptorSetLayoutBinding& currentBinding :
             shader->GetDescriptorSetLayoutBindings())
        {
                                         ...
                                         const char* name)
{
    ...
    VULKAN_API_CALL(vkAllocateDescriptorSets(device->GetGraphicDevice(), &descriptorSetAllocateInfo,
                                             &m_DescriptorSet));
    ...
    for (uint32_t i = 0; i < descriptors.size(); ++i)
    {
        ...
        case DescriptorType::CombinedSampler:
            ...
            writeDescriptorSet.pImageInfo = new VkDescriptorImageInfo(
                descriptors[i].GetDescriptorResourceInfo().m_ResourceInfo.ImageInfo);
            break;
        case DescriptorType::SampledImage:
        ...
        case DescriptorType::UniformBuffer:
            ...
            writeDescriptorSet.pBufferInfo = new VkDescriptorBufferInfo(
                descriptors[i].GetDescriptorResourceInfo().m_ResourceInfo.BufferInfo);
            break;
        case DescriptorType::StorageImage:
        ...
        case DescriptorType::AccelerationStructure:
            ...
            descriptorAccelerationStructureInfo->pAccelerationStructures =
                new VkAccelerationStructureKHR(descriptors[i]
                                                   .GetDescriptorResourceInfo()
                                                   .m_ResourceInfo.AccelerationStructureHandle);
    ...
    vkUpdateDescriptorSets(device->GetGraphicDevice(),
