...
VulkanSwapchain::~VulkanSwapchain()
{
    ...
    {
        vkDestroySemaphore(m_VulkanDevice.GetGraphicDevice(), m_RenderFinishedSemaphores[i],
                           nullptr);
        vkDestroySemaphore(m_VulkanDevice.GetGraphicDevice(), m_ImageAvailableSemaphores[i],
                           nullptr);
        vkDestroyFence(m_VulkanDevice.GetGraphicDevice(), m_InFlightFences[i], nullptr);
    }

    for (auto object : m_SwapChainVulkanImageViews)
    {
        delete (object);
    }
}
...
VkResult VulkanSwapchain::AcquireNextImage(uint32_t* imageIndex)
{
    ...
    vkWaitForFences(m_VulkanDevice.GetGraphicDevice(), 1, &m_InFlightFences[m_CurrentFrame],
                    VK_TRUE, std::numeric_limits<uint64_t>::max());

    VkResult result = vkAcquireNextImageKHR(
        m_VulkanDevice.GetGraphicDevice(), m_SwapChain, std::numeric_limits<uint64_t>::max(),
...
VkResult VulkanSwapchain::SubmitCommandBufferAndPresent(VulkanCommandBuffer& buffer,
                                                        uint32_t* imageIndex)
{
    OPTICK_EVENT();
    ...
    {
        vkWaitForFences(m_VulkanDevice.GetGraphicDevice(), 1, &m_ImagesInFlight[*imageIndex],
                        VK_TRUE, UINT64_MAX);
    }
    ...
    VULKAN_API_CALL(vkQueueSubmit(m_VulkanDevice.GetGraphicsQueue(), 1, &submitInfo,
...
void VulkanSwapchain::CreateSwapChain(VkSwapchainKHR oldSwapChain)
{
    ...
    VULKAN_API_CALL(vkCreateSwapchainKHR(m_VulkanDevice.GetGraphicDevice(), &createInfo, nullptr,
                                         &m_SwapChain));

    vkGetSwapchainImagesKHR(m_VulkanDevice.GetGraphicDevice(), m_SwapChain, &imageCount, nullptr);
    m_SwapChainImages.resize(imageCount);
    vkGetSwapchainImagesKHR(m_VulkanDevice.GetGraphicDevice(), m_SwapChain, &imageCount,
...
void VulkanSwapchain::CreateImageViews()
{
    ...
    for (size_t i = 0; i < m_SwapChainVulkanImages.size(); i++)
    {
        ...
        m_SwapChainVulkanImageViews[i] =
            new VulkanImageView(&m_VulkanDevice, imageViewInfo, "swapchain image views");
    }
}
...
void VulkanSwapchain::CreateSyncObjects()
{
    ...
    {
        VULKAN_API_CALL(vkCreateSemaphore(m_VulkanDevice.GetGraphicDevice(), &semaphoreInfo,
                                          nullptr, &m_ImageAvailableSemaphores[i]));
        VULKAN_API_CALL(vkCreateSemaphore(m_VulkanDevice.GetGraphicDevice(), &semaphoreInfo,
                                          nullptr, &m_RenderFinishedSemaphores[i]));
        VULKAN_API_CALL(vkCreateFence(m_VulkanDevice.GetGraphicDevice(), &fenceInfo, nullptr,
                                      &m_InFlightFences[i]));
    }
}
