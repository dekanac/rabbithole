...
VulkanTexture::VulkanTexture(VulkanDevice& device, RWTextureCreateInfo& createInfo)
    : ManagableResource(ResourceType::Texture), m_Format(createInfo.format),
      m_Flags(createInfo.flags), m_Name(createInfo.name)
{
    ...
    device.SetObjectName((uint64_t)GET_VK_HANDLE_PTR(m_Resource), VK_OBJECT_TYPE_IMAGE,
                         createInfo.name.c_str());
    device.SetObjectName((uint64_t)GET_VK_HANDLE_PTR(m_View), VK_OBJECT_TYPE_IMAGE_VIEW,
                         createInfo.name.c_str());
    device.SetObjectName((uint64_t)GET_VK_HANDLE_PTR(m_Sampler), VK_OBJECT_TYPE_SAMPLER,
                         createInfo.name.c_str());
}

VulkanTexture::VulkanTexture(VulkanDevice& device, const TextureData* data,
                             ROTextureCreateInfo& createInfo)
    : ManagableResource(ResourceType::Texture), m_Format(createInfo.format),
      m_Flags(createInfo.flags), m_Name(createInfo.name)
{
...
VulkanTexture::VulkanTexture(VulkanDevice& device, const VulkanTexture* other, uint32_t mipSlice)
    : ManagableResource((ManagableResource)*other), m_Resource(other->m_Resource),
      m_Sampler(other->m_Sampler), m_Format(other->m_Format), m_Flags(other->m_Flags),
...
void VulkanTexture::CreateResource(VulkanDevice* device, const TextureData* texData,
                                   ROTextureCreateInfo& createInfo)
{
    bool isCubeMap = IsFlagSet(m_Flags & TextureFlags::CubeMap);

    bool generateMipsLater = createInfo.generateMips;
    // decide whether we generate mips or we load (mipCount) already generated
    // mips from file
    uint32_t mipCount = generateMipsLater ? GET_MIP_LEVELS_FROM_RES(texData->width, texData->height)
                                          : createInfo.mipCount;
    uint32_t arraySize = isCubeMap ? 6 : 1;
    Extent3D textureExtent =
        Extent3D{static_cast<uint32_t>(texData->width), static_cast<uint32_t>(texData->height), 1};

    InitializeRegion(textureExtent, arraySize, mipCount);

    uint64_t textureSize =
        GetTextureSizeFrom(m_Format, textureExtent, generateMipsLater ? 1 : mipCount, arraySize);

    VulkanBuffer stagingBuffer(*device, BufferCreateInfo{.flags = BufferUsageFlags::TransferSrc,
    ...
    textureResourceInfo.Flags =
        (isCubeMap ? ImageFlags::CubeMap : ImageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::LinearTiling) ? ImageFlags::LinearTiling
                                                         : ImageFlags::None);
    textureResourceInfo.UsageFlags =
        ImageUsageFlags::Resource |
        (IsFlagSet(m_Flags & TextureFlags::TransferDst) ? ImageUsageFlags::TransferDst
                                                        : ImageUsageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::TransferSrc) ? ImageUsageFlags::TransferSrc
                                                        : ImageUsageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::DepthStencil) ? ImageUsageFlags::DepthStencil
                                                         : ImageUsageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::RenderTarget) ? ImageUsageFlags::RenderTarget
                                                         : ImageUsageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::Storage) ? ImageUsageFlags::Storage
                                                    : ImageUsageFlags::None);
    ...
    device->ResourceBarrier(tempCommandBuffer, this, ResourceState::None,
                            ResourceState::TransferDst, ResourceStage::Undefined,
                            ResourceStage::Transfer, 0, mipCount);
    ...
    {
        device->CopyBufferToImage(tempCommandBuffer, &stagingBuffer, this,
                                  generateMipsLater ? 1 : mipCount);
    }
    ...
    {
        device->ResourceBarrier(tempCommandBuffer, this, ResourceState::TransferDst, stateAfter,
                                ResourceStage::Transfer, ResourceStage::Undefined);
    }
...
void VulkanTexture::CreateResource(VulkanDevice* device, RWTextureCreateInfo& createInfo)
{
    ...
    VulkanImageInfo textureResourceInfo;
    textureResourceInfo.Flags =
        (IsFlagSet(m_Flags & TextureFlags::CubeMap) ? ImageFlags::CubeMap : ImageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::LinearTiling) ? ImageFlags::LinearTiling
                                                         : ImageFlags::None);
    ...
    textureResourceInfo.UsageFlags =
        ...
        (IsFlagSet(m_Flags & TextureFlags::DepthStencil) ? ImageUsageFlags::DepthStencil
                                                         : ImageUsageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::Read) ? ImageUsageFlags::Resource
                                                 : ImageUsageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::RenderTarget) ? ImageUsageFlags::RenderTarget
                                                         : ImageUsageFlags::None) |
        (IsFlagSet(m_Flags & TextureFlags::Storage) ? ImageUsageFlags::Storage
    ...
    device->ResourceBarrier(tempCommandBuffer, this, ResourceState::None, stateAfter,
                            ResourceStage::Undefined, ResourceStage::Undefined, 0,
                            m_Region.Subresource.MipSize);
...
void VulkanTexture::CreateSampler(VulkanDevice* device, SamplerType type, AddressMode addressMode)
{
    ...
    imageSamplerInfo.MagFilterType =
        (type == SamplerType::Point) ? FilterType::Point : FilterType::Linear;
    imageSamplerInfo.MinFilterType =
        (type == SamplerType::Point) ? FilterType::Point : FilterType::Linear;
    imageSamplerInfo.MipFilterType =
        (type == SamplerType::Point) ? FilterType::Point : FilterType::Linear;
...
void VulkanTexture::GenerateMips(VulkanCommandBuffer& commandBuffer, VulkanDevice* device,
                                 uint32_t mipCount)
{
    // Check if image format supports linear blitting
    VkFormatProperties formatProperties;
    vkGetPhysicalDeviceFormatProperties(device->GetPhysicalDevice(), GetVkFormatFrom(m_Format),
                                        &formatProperties);

    ASSERT(formatProperties.optimalTilingFeatures &
               VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT,
           "Format doesn't support linear filtering and you're trying to "
           "generate mips");
    ...
    {
        device->ResourceBarrier(commandBuffer, this, ResourceState::TransferDst,
                                ResourceState::TransferSrc, ResourceStage::Transfer,
                                ResourceStage::Transfer, i - 1, 1);
        ...
        vkCmdBlitImage(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE_PTR(m_Resource),
                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, GET_VK_HANDLE_PTR(m_Resource),
                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_LINEAR);

        device->ResourceBarrier(commandBuffer, this, ResourceState::TransferSrc,
                                m_ShouldBeResourceState, ResourceStage::Transfer,
                                ResourceStage::Undefined, i - 1, 1);
    ...
    device->ResourceBarrier(commandBuffer, this, ResourceState::TransferDst,
                            m_ShouldBeResourceState, ResourceStage::Transfer,
                            ResourceStage::Undefined, mipCount - 1, 1);
}
