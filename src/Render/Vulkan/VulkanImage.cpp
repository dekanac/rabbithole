...
VulkanImage::VulkanImage(const VulkanDevice* device, const VulkanImageInfo& info, const char* name)
    : m_VulkanDevice(device), m_Info(info), m_Format(GetVkFormatFrom(m_Info.Format)),
      m_ImageType(GetVkImageTypeFrom(m_Info.Extent.Depth))
{
    VkImageCreateInfo imageCreateInfo = {VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO};

    imageCreateInfo.flags |=
        IsFlagSet(m_Info.Flags & ImageFlags::CubeMap) ? VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT : 0;
    ...
    VULKAN_API_CALL(vmaCreateImage(m_VulkanDevice->GetVmaAllocator(), &imageCreateInfo,
                                   &allocationCreateInfo, &m_Image, &m_Allocation, nullptr));
}

VulkanImage::VulkanImage(const VulkanDevice* device, const VulkanSwapchain* swapchain,
                         const uint32_t backBufferIndex)
{
    m_Info.Flags = ImageFlags::None;
    m_Info.UsageFlags = ImageUsageFlags::TransferDst | ImageUsageFlags::Resource |
                        ImageUsageFlags::Storage | ImageUsageFlags::RenderTarget;
    m_Info.MemoryAccess = MemoryAccess::GPU;
    m_Info.Format = Format::B8G8R8A8_UNORM_SRGB; // swapchain->GetSwapChainImageFormat();
                                                 // //TODO: add proper fields to
                                                 // swapchain class
...
VulkanImageView::VulkanImageView(const VulkanDevice* device, const VulkanImageViewInfo& info,
                                 const char* name)
    : m_VulkanDevice(device), m_Info(info)
{
    ...
    {
        imageViewCreateInfo.viewType = info.Resource->GetInfo().ArraySize > 1
    ...
    imageViewCreateInfo.subresourceRange.aspectMask =
        GetVkImageAspectFlagsFrom(GetVkFormatFrom(m_Info.Format));
    ...
    VULKAN_API_CALL(vkCreateImageView(m_VulkanDevice->GetGraphicDevice(), &imageViewCreateInfo,
                                      nullptr, &m_ImageView));
}
...
VulkanImageSampler::VulkanImageSampler(const VulkanDevice* device,
                                       const VulkanImageSamplerInfo& info, const char* name)
    : m_VulkanDevice(device), m_Info(info)
{
    ...
    VULKAN_API_CALL(vkCreateSampler(m_VulkanDevice->GetGraphicDevice(), &samplerCreateInfo, nullptr,
                                    &m_Sampler));
}
