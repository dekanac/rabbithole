    ...
    if (size > 0)
    {
        ...
        else
        {
            VulkanBuffer stagingBuffer(m_Device,
                                       BufferCreateInfo{.flags = BufferUsageFlags::TransferSrc,
            ...
            // TODO: no way! Temp Command Buffers should be used only in init
            // phases!!!
...
void VulkanBuffer::FillBuffer(void* data) { FillBuffer(data, GetSize(), 0); }
