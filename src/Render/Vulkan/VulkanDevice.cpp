...
void VulkanDevice::CreateLogicalDevice()
{
    ...
    VkPhysicalDeviceShaderFloat16Int8FeaturesKHR float16Feature{
...
{
    if (glfwCreateWindowSurface(m_Instance, Window::instance().GetNativeWindowHandle(), nullptr,
                                &m_PresetingSurface) != VK_SUCCESS)
    {
...
SwapChainSupportDetails VulkanDevice::QuerySwapChainSupport(VkPhysicalDevice device)
{
    ...
    if (formatCount != 0)
    {
        ...
        vkGetPhysicalDeviceSurfaceFormatsKHR(device, m_PresetingSurface, &formatCount,
                                             details.formats.data());
    }

    uint32_t presentModeCount;
    vkGetPhysicalDeviceSurfacePresentModesKHR(device, m_PresetingSurface, &presentModeCount,
                                              nullptr);
...
{
    pfnDebugMarkerSetObjectTag = (PFN_vkDebugMarkerSetObjectTagEXT)vkGetInstanceProcAddr(
        m_Instance, "vkDebugMarkerSetObjectTagEXT");
    pfnDebugUtilsObjectName = (PFN_vkSetDebugUtilsObjectNameEXT)vkGetDeviceProcAddr(
...
void VulkanDevice::CopyBuffer(VulkanCommandBuffer& commandBuffer, VulkanBuffer& srcBuffer,
                              VulkanBuffer& dstBuffer, uint64_t size, uint64_t srcOffset,
                              uint64_t dstOffset)
{
    ...
    vkCmdCopyBuffer(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE(srcBuffer),
                    GET_VK_HANDLE(dstBuffer), 1, &copyRegion);
}

void VulkanDevice::CopyBufferToImage(VulkanCommandBuffer& commandBuffer, VulkanBuffer* buffer,
                                     VulkanTexture* texture, uint32_t mipCount)
{
    ...
    for (uint32_t i = 0; i < mipCount; i++)
    {
        ...
        offset += static_cast<uint32_t>(
            GetTextureSizeFrom(texture->GetFormat(), {width >> i, height >> i, 1}, 1, 1));
    }

    vkCmdCopyBufferToImage(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE_PTR(buffer),
...
void VulkanDevice::CopyBufferToImageCubeMap(VulkanCommandBuffer& commandBuffer,
                                            VulkanBuffer* buffer, VulkanTexture* texture)
{
    ImageRegion texRegion = texture->GetRegion();
...
void VulkanDevice::CopyImage(VulkanCommandBuffer& commandBuffer, VulkanTexture* src,
                             VulkanTexture* dst)
{
    VkImageCopy imageCopyRegion{};
    imageCopyRegion.srcSubresource.aspectMask =
        GetVkImageAspectFlagsFrom(GetVkFormatFrom(src->GetFormat()));
    imageCopyRegion.srcSubresource.layerCount = src->GetRegion().Subresource.ArraySize;
    imageCopyRegion.dstSubresource.aspectMask =
        GetVkImageAspectFlagsFrom(GetVkFormatFrom(dst->GetFormat()));
    ...
    if (srcState != ResourceState::TransferSrc)
        ResourceBarrier(commandBuffer, src, srcState, ResourceState::TransferSrc, srcStage,
                        ResourceStage::Transfer);
    if (dstState != ResourceState::TransferDst)
        ResourceBarrier(commandBuffer, dst, dstState, ResourceState::TransferDst, dstStage,
                        ResourceStage::Transfer);

    vkCmdCopyImage(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE_PTR(src->GetResource()),
                   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, GET_VK_HANDLE_PTR(dst->GetResource()),
                   VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageCopyRegion);

    if (srcState != ResourceState::TransferSrc)
        ResourceBarrier(commandBuffer, src, ResourceState::TransferSrc, srcState,
                        ResourceStage::Transfer, srcStage);
    if (dstState != ResourceState::TransferDst)
        ResourceBarrier(commandBuffer, dst, ResourceState::TransferDst, dstState,
                        ResourceStage::Transfer, dstStage);
}
...
void VulkanDevice::ResourceBarrier(VulkanCommandBuffer& commandBuffer, VulkanTexture* texture,
...
void VulkanDevice::ResourceBarrier(VulkanCommandBuffer& commandBuffer, VulkanBuffer* buffer,
                                   ...
                                   ResourceStage srcStage, ResourceStage dstStage)
{
    ASSERT(oldLayout != ResourceState::Count && newLayout != ResourceState::Count,
           "Invalid layouts!");
    ...
    if (oldLayout != ResourceState::None)
    {
        ...
        vkCmdPipelineBarrier(
            ...
            GetVkPipelineStageFromResourceStageAndState(srcStage, oldLayout, true),
            GetVkPipelineStageFromResourceStageAndState(dstStage, newLayout, false), 0, 0, nullptr,
...
void VulkanDevice::CopyImageToBuffer(VulkanCommandBuffer& commandBuffer, VulkanTexture* texture,
                                     VulkanBuffer* buffer)
{
    ImageRegion texRegion = texture->GetRegion();
    ...
    if (srcState != ResourceState::TransferSrc)
        ResourceBarrier(commandBuffer, texture, srcState, ResourceState::TransferSrc, srcStage,
                        ResourceStage::Transfer);

    vkCmdCopyImageToBuffer(GET_VK_HANDLE(commandBuffer), GET_VK_HANDLE_PTR(texture->GetResource()),
                           VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, GET_VK_HANDLE_PTR(buffer), 1,
                           &region);

    if (srcState != ResourceState::TransferSrc)
        ResourceBarrier(commandBuffer, texture, ResourceState::TransferSrc, srcState,
                        ResourceStage::Transfer, srcStage);
}
