...
{
    for (auto object : m_GraphicPipelines)
    {
        delete object.second;
    }
    for (auto object : m_ComputePipelines)
    {
        delete object.second;
    }
    for (auto object : m_RayTracingPipelines)
    {
        delete object.second;
    }
    for (auto object : m_RenderPasses)
    {
        delete object.second;
    }
}

VulkanPipeline* PipelineManager::FindOrCreateGraphicsPipeline(VulkanDevice& device,
                                                              PipelineInfo& pipelineInfo)
{
    GraphicsPipelineKey key{};

    key.vertexShaderCRC = pipelineInfo.vertexShader ? pipelineInfo.vertexShader->GetHash() : 0;
    key.vertexShaderEntryPointCRC = (uint32_t)crc32_fast(
        (const void*)pipelineInfo.vsEntryPoint.c_str(), pipelineInfo.vsEntryPoint.length());
    key.pixelShaderCRC = pipelineInfo.pixelShader ? pipelineInfo.pixelShader->GetHash() : 0;
    key.pixelShaderEntryPointCRC = (uint32_t)crc32_fast(
        (const void*)pipelineInfo.psEntryPoint.c_str(), pipelineInfo.psEntryPoint.length());
    ...
    {
        LOG_WARNING("If you're seeing this every frame, you're doing something "
                    "wrong! Check GraphicsPipelineKey!");
...
VulkanPipeline* PipelineManager::FindOrCreateComputePipeline(VulkanDevice& device,
                                                             PipelineInfo& pipelineInfo)
{
    // for now the key is only CRC of compute shader and CRC of string of entry
    // point
    ComputePipelineKey key{};

    key.computeShaderCRC = pipelineInfo.computeShader->GetHash();
    key.computeShaderEntryPointCRC = (uint32_t)crc32_fast(
        (const void*)pipelineInfo.csEntryPoint.c_str(), pipelineInfo.csEntryPoint.length());
    ...
    {
        LOG_WARNING("If you're seeing this every frame, you're doing something "
                    "wrong! Check ComputePipelineKey!");
...
VulkanPipeline* PipelineManager::FindOrCreateRayTracingPipeline(VulkanDevice& device,
                                                                PipelineInfo& pipelineInfo)
{
    ...
    {
        LOG_WARNING("If you're seeing this every frame, you're doing something "
                    "wrong! Check RayTracingPipelineKey!");
...
RenderPass* PipelineManager::FindOrCreateRenderPass(
    VulkanDevice& device, const std::vector<VulkanImageView*>& renderTargets,
    const VulkanImageView* depthStencil, RenderPassInfo& renderPassInfo)
{
    RenderPassKey key{};
    ...
    for (size_t i = 0; i < renderTargets.size(); i++)
    {
        ...
        key.attachmentDescriptions[i].samples =
            GetVkSampleFlagsFrom(renderTargets[i]->GetInfo().Resource->GetInfo().MultisampleType);
        ...
        key.attachmentDescriptions[i].initialLayout =
            GetVkImageLayoutFrom(renderPassInfo.InitialRenderTargetState);
        key.attachmentDescriptions[i].finalLayout =
            GetVkImageLayoutFrom(renderPassInfo.FinalRenderTargetState);
    }
    ...
    if (depthStencil != nullptr)
    {
        ...
        key.depthStencilAttachmentDescription.samples =
            GetVkSampleFlagsFrom(depthStencil->GetInfo().Resource->GetInfo().MultisampleType);
        ...
        key.depthStencilAttachmentDescription.initialLayout =
            GetVkImageLayoutFrom(renderPassInfo.InitialDepthStencilState);
        key.depthStencilAttachmentDescription.finalLayout =
            GetVkImageLayoutFrom(renderPassInfo.FinalDepthStencilState);
    }
    ...
    {
        LOG_WARNING("If you're seeing this every frame, you're doing something "
                    "wrong! Check RenderPassKey!");
        RenderPass* newRenderPass =
            new RenderPass(device, renderTargets, depthStencil, renderPassInfo, "RenderPass");
...
VulkanDescriptorSet*
PipelineManager::FindOrCreateDescriptorSet(VulkanDevice& device,
                                           ...
                                           const std::vector<VulkanDescriptor>& descriptors)
{
    ...
    {
        LOG_WARNING("If you're seeing this every frame, you're doing something "
                    "wrong! Check DescriptorSetKey!");
        VulkanDescriptorSet* newDescSet = new VulkanDescriptorSet(
            &device, desciptorPool, descriptorSetLayout, descriptors, "descriptorset");
...
{
    vkCmdBindPipeline(GET_VK_HANDLE(commandBuffer), VK_PIPELINE_BIND_POINT_RAY_TRACING_KHR,
                      m_Pipeline);
}
