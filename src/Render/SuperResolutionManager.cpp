...
void SuperResolutionManager::Destroy() { DestroyFsrContext(); }
...
void SuperResolutionManager::CreateFsrContext(VulkanDevice* device)
{
    ...
    FfxErrorCode errorCode =
        ffxFsr2GetInterfaceVK(&m_FsrContextDescription.callbacks, scratchBuffer, scratchBufferSize,
...
void SuperResolutionManager::DestroyFsrContext() { ffxFsr2ContextDestroy(&m_FsrContext); }
...
void SuperResolutionManager::PreDraw(UIState* pState)
{
    ...
    if (m_UseTaa)
    {
        ...
        const int32_t jitterPhaseCount =
            ffxFsr2GetJitterPhaseCount(m_NativeResolutionWidth, m_UpscaledResolutionWidth);
        ffxFsr2GetJitterOffset(&m_JitterX, &m_JitterY, m_Index, jitterPhaseCount);

        pState->camera->SetProjectionJitter(2.0f * m_JitterX / (float)pState->renderWidth,
                                            -2.0f * m_JitterY / (float)pState->renderHeight);
    }
    else
    {
...
void SuperResolutionManager::Draw(VulkanCommandBuffer& commandBuffer,
                                  const FfxUpscaleSetup& cameraSetup, UIState* pState)
{
    ...
    dispatchParameters.color = ffxGetTextureResourceVK(
        &m_FsrContext, GET_VK_HANDLE_PTR(cameraSetup.unresolvedColorResource->GetResource()),
        GET_VK_HANDLE_PTR(cameraSetup.unresolvedColorResource->GetView()),
        cameraSetup.unresolvedColorResource->GetWidth(),
        cameraSetup.unresolvedColorResource->GetHeight(),
        GetVkFormatFrom(cameraSetup.unresolvedColorResource->GetFormat()),
        (wchar_t*)L"FSR2_InputColor");
    dispatchParameters.depth = ffxGetTextureResourceVK(
        &m_FsrContext, GET_VK_HANDLE_PTR(cameraSetup.depthbufferResource->GetResource()),
        GET_VK_HANDLE_PTR(cameraSetup.depthbufferResource->GetView()),
        cameraSetup.depthbufferResource->GetWidth(), cameraSetup.depthbufferResource->GetHeight(),
        GetVkFormatFrom(cameraSetup.depthbufferResource->GetFormat()),
        (wchar_t*)L"FSR2_InputDepth");
    dispatchParameters.motionVectors = ffxGetTextureResourceVK(
        &m_FsrContext, GET_VK_HANDLE_PTR(cameraSetup.motionvectorResource->GetResource()),
        GET_VK_HANDLE_PTR(cameraSetup.motionvectorResource->GetView()),
        cameraSetup.motionvectorResource->GetWidth(), cameraSetup.motionvectorResource->GetHeight(),
        GetVkFormatFrom(cameraSetup.motionvectorResource->GetFormat()),
        (wchar_t*)L"FSR2_InputMotionVectors");
    dispatchParameters.output = ffxGetTextureResourceVK(
        &m_FsrContext, GET_VK_HANDLE_PTR(cameraSetup.resolvedColorResource->GetResource()),
        GET_VK_HANDLE_PTR(cameraSetup.resolvedColorResource->GetView()),
        cameraSetup.resolvedColorResource->GetWidth(),
        cameraSetup.resolvedColorResource->GetHeight(),
        GetVkFormatFrom(cameraSetup.resolvedColorResource->GetFormat()),
        (wchar_t*)L"FSR2_OutputUpscaledColor", FFX_RESOURCE_STATE_UNORDERED_ACCESS);
...
void SuperResolutionManager::GenerateReactiveMask(VulkanCommandBuffer& commandBuffer,
                                                  const FfxUpscaleSetup& cameraSetup,
                                                  UIState* pState)
{
