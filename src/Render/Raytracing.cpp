...
{
    pfnvkCreateAccelerationStructureKHR = reinterpret_cast<PFN_vkCreateAccelerationStructureKHR>(
        vkGetDeviceProcAddr(device.GetGraphicDevice(), "vkCreateAccelerationStructureKHR"));
    pfnvkGetAccelerationStructureDeviceAddressKHR =
        reinterpret_cast<PFN_vkGetAccelerationStructureDeviceAddressKHR>(vkGetDeviceProcAddr(
            device.GetGraphicDevice(), "vkGetAccelerationStructureDeviceAddressKHR"));
}
...
void CreateAccelerationStructure(Renderer* renderer, AccelerationStructure& accelerationStructure,
                                 VkAccelerationStructureTypeKHR type,
                                 VkAccelerationStructureBuildSizesInfoKHR buildSizeInfo)
{
    VulkanBuffer* asBuffer = renderer->GetResourceManager().CreateBuffer(
        renderer->GetVulkanDevice(),
        BufferCreateInfo{.flags = {BufferUsageFlags::AccelerationStructureStorage |
    ...
    pfnvkCreateAccelerationStructureKHR(renderer->GetVulkanDevice().GetGraphicDevice(),
    ...
    accelerationDeviceAddressInfo.accelerationStructure =
        accelerationStructure.accelerationStructure;
    accelerationStructure.deviceAddress = pfnvkGetAccelerationStructureDeviceAddressKHR(
        renderer->GetVulkanDevice().GetGraphicDevice(), &accelerationDeviceAddressInfo);
}
...
RayTracing::ShaderBindingTable CreateShaderBindingTable(VulkanDevice& device, uint32_t handleCount)
{
    ...
    newTable.buffer = resourceManager.CreateBuffer(
        device, BufferCreateInfo{.flags = {BufferUsageFlags::ShaderBindingTable |
                                           BufferUsageFlags::ShaderDeviceAddress},
                                 .memoryAccess = {MemoryAccess::CPU},
                                 .size = {shaderGroupHandleSize * handleCount},
                                 .name = {"Table Buffer"}});
    newTable.stridedDeviceAddressRegion =
        RayTracing::GetSbtEntryStridedDeviceAddressRegion(device, newTable.buffer, handleCount);
    newTable.buffer->Map();

    return newTable;
}

VkStridedDeviceAddressRegionKHR GetSbtEntryStridedDeviceAddressRegion(VulkanDevice& device,
                                                                      VulkanBuffer* buffer,
                                                                      uint32_t handleCount)
{
    const uint64_t handleSizeAligned =
        AlignedSize(device.GetRayTracingProperties().shaderGroupHandleSize,
                    device.GetRayTracingProperties().shaderGroupHandleAlignment);
