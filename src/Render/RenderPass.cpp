...
RenderPass::RenderPass(VulkanDevice& device, const std::vector<VulkanImageView*> renderTargetViews,
                       ...
                       const char* name)
    : m_Device(device), m_Extent({info.extent})
{
    m_RenderPass = new VulkanRenderPass(
        &device, renderTargetViews, depthStencilView,
        VulkanRenderPassInfo{.ClearRenderTargets = info.ClearRenderTargets,
    ...
    m_Framebuffer = new VulkanFramebuffer(
        &device, VulkanFramebufferInfo{.width = info.extent.width, .height = info.extent.height},
