...
struct IndexedIndirectBuffer
{
    ...
    void SubmitToGPU();
    void AddIndirectDrawCommand(VulkanCommandBuffer& commandBuffer,
...
class Renderer
{
  ...
  private:
    ...
    void CompileShader(const std::string& name, const std::string& entryPoint = "main",
  ...
  public:
    ...
    inline ResourceStateTrackingManager& GetResourceStateTrackingManager()
    {
        return m_ResourceStateTrackingManager;
    }
    ...
    inline VulkanBuffer* GetMainConstBufferByIndex(uint32_t imageIndex)
    {
        return m_MainConstBuffer[imageIndex];
    }

    void ResourceBarrier(VulkanTexture* texture, ResourceState oldLayout, ResourceState newLayout,
                         ResourceStage srcStage, ResourceStage dstStage, uint32_t mipLevel = 0,
                         uint32_t mipCount = UINT32_MAX);
    void ResourceBarrier(VulkanBuffer* buffer, ResourceState oldLayout, ResourceState newLayout,
    ...
    void CopyBuffer(VulkanBuffer& src, VulkanBuffer& dst, uint64_t size = UINT64_MAX,
    ...
    inline VulkanTexture* GetTextureWithID(uint32_t textureId)
    {
        return m_ResourceManager.GetTextures()[textureId];
    }
    inline Camera& GetCamera() { return m_MainCamera; }
    inline UIState& GetUIState() { return m_CurrentUIState; }
    inline CameraState& GetCameraState() { return m_CurrentCameraState; }
    inline bool IsImguiReady()
    {
        return m_ImGuiManager.IsInitialized() && m_ImGuiManager.IsReady();
    }
    inline const std::string& GetResFolderPath() { return m_ResFolder; }

    template <typename T> void BindPushConst(T& data)
    {
        auto size = sizeof(T);
    ...
    {
        return *m_MainRenderCommandBuffers[m_CurrentImageIndex];
    }
  ...
  public:
    // Don't ask, Imgui init wants swapchain renderpass to be ready, but its
    // not. So basically we need 2 init phases..
...
#endif
  private:
    void CalculateMatrices(VulkanglTFModel::Node* node, Vertex* vertexBufferCpu,
