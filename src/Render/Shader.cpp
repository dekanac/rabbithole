...
Shader::Shader(VulkanDevice& device, size_t byteCodeSize, const char* byteCode,
               const ShaderInfo& info, const char* name)
    : m_Device(device), m_Info(info), m_Name(name)
{
    VkShaderModuleCreateInfo shaderCreateInfo = {VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};
    shaderCreateInfo.codeSize = byteCodeSize;
    shaderCreateInfo.pCode = reinterpret_cast<const uint32_t*>(byteCode);

    VULKAN_API_CALL(vkCreateShaderModule(m_Device.GetGraphicDevice(), &shaderCreateInfo, nullptr,
                                         &m_ShaderModule));
    ...
    for (uint32_t i = 0; i < bindingCount; ++i)
    {
        ...
        descriptorSetLayoutBinding.descriptorType =
            GetVkDescriptorTypeFrom(bindings[i]->descriptor_type);
    ...
    VULKAN_SPIRV_CALL(
        spvReflectEnumeratePushConstantBlocks(&spvModule, &pushConstsCount, pushConstants.data()));
...
Shader::~Shader() { vkDestroyShaderModule(m_Device.GetGraphicDevice(), m_ShaderModule, nullptr); }
